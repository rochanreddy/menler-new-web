...
export async function createEnrolOrder({ program, leadId, name, email, phone, city, background, college, graduation_year, track, pdf, resource, pack }) {
  const res = await fetch(`${API_URL}/payments/cashfree/order`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ program, leadId, name, email, phone, city, background, college, graduation_year, track, pdf, resource, pack }),
  });
