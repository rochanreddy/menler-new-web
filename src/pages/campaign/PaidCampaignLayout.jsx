/* The paid campaign design, as a component.
 *
 * Everything a visitor sees lives here; everything a campaign says lives in the
 * `content` object handed in. Two pages pass their own constants, and
 * /campaign/:slug passes a Sanity document mapped to the same shape — so a new
 * paid campaign is content, not another copy of this file.
 */
...
/* Three WhatsApp screens, drawn in HTML from the campaign's own community
   content. Pass `shot` to use a real screenshot instead. */
function Phone({ kind, shot, dateShort, timeShort, community }) {
  if (shot) return <img className="aw-phone" src={shot} alt="" loading="lazy" style={{ display: 'block', width: '100%' }} />;
  const when = `${dateShort}, ${timeShort}`;
  const head = (
    <>
      ...
      <div className="aw-phone-body" style={{ background: '#fff', padding: 0 }}>
        <div className="aw-list">
          {community.groups.map((g) => (
            <div key={g.name}><i>{g.icon}</i><div><b>{g.name}</b><small>{g.sub}</small></div><em>{g.count}</em></div>
          ))}
        </div>
      </div>
    </div>
    ...
    <div className="aw-phone">
      ...
      <div className="aw-phone-body">
        {community.announcements.map((a) => (
          <div className="aw-msg" key={a.meta}>
            <b>Menler (Admin)</b>{a.body.replace('{when}', when)}
            {a.file && <span className="file"><i>{a.file.kind}</i>{a.file.name}</span>}
      ...
      <div className="aw-phone-body" style={{ background: '#fff', padding: 0 }}>
        <div className="aw-list">
          {community.resources.map((r) => (
            <div key={r.name}><i>{r.kind}</i><div><b>{r.name}</b><small>{r.sub}</small></div></div>
          ))}
        </div>
      </div>
    </div>
...
export default function PaidCampaignLayout({ content }) {
  // Destructured into the same names the markup already uses, so the design
  // below is identical whether the content is a constant in a page file or
  // a document out of Sanity.
  const {
    SLUG,
    SUBTITLE,
    ROOT_CLASS,
  ...
  return (
    <div className={ROOT_CLASS}>
      ...
      <section className="aw-audience">
        ...
        <p className="aw-audience-foot"><span><i>+</i> {AUDIENCE_FOOT}</span></p>
      </section>
      ...
      <section className="aw-community">
        <div className="aw-cm-head">
          ...
          <p className="aw-cm-lede">{COMMUNITY.lede}</p>
        </div>
        <div className="aw-cm-shots">
          <figure><Phone kind="groups" community={COMMUNITY} dateShort={SESSION.dateShort} timeShort={SESSION.timeShort} /><figcaption><b>Topic groups</b><span>{COMMUNITY.groupsLabel}</span></figcaption></figure>
          <figure className="mid"><Phone kind="announcements" community={COMMUNITY} dateShort={SESSION.dateShort} timeShort={SESSION.timeShort} /><figcaption><b>Announcements</b><span>Prompt packs &amp; tool drops</span></figcaption></figure>
          <figure><Phone kind="resources" community={COMMUNITY} dateShort={SESSION.dateShort} timeShort={SESSION.timeShort} /><figcaption><b>Resources</b><span>Files, links, voice notes</span></figcaption></figure>
        </div>
        ...
        <div className="aw-pb-grid">
          ...
          <div className="aw-pb-buy" id="checkout">
            <div className="aw-price-card">
              ...
              <div className="aw-tiny-note">{TINY_NOTE}</div>
            </div>
          </div>
        </div>
      </section>
